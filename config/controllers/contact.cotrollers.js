exports.addContact=async(req,res)=>{try {
    ...
    if(findContact){
        ...
        res.status(200).send({msg:"Add contact succ",newContact});  
    } catch (error) {
...
exports.getAllContact =async(req,res)=>{
    try {const listContact=await Contact.find();
        res.status(200).send({msg:"api all the contact", listContact});
...
exports.getContact =async(req, res)=>{
    try {
        ...
        res.status(200).send({msg:"get it success",FindContact});
    } catch (error) {
